...
var APIKey = "ADD YOUR API KEY HERE - LIMITED KEY SO CAN READ FACTION STUFF";
...
async function putCrimeInfoIntoTable(_crimeArray, _afterElm) {
  ...
  if (_crimeArray.length > 0) {
    for (let i = 0; i < _crimeArray.length; i++) {
      ...
      // ===== Mapping for % values =====
      const mappedPercentValues = {
      "Blast from the Past": {
        "Muscle": "34.6%",       // no number, because Muscle isn't numbered in this crime
        "Bomber": "15.6%",       // no number
        "Engineer": "24.0%",
        "Hacker": "12.1%",
        "Picklock 1": "10.8%",
        "Picklock 2": "2.9%"
      ...
      "Break the Bank": {
        ...
        "Muscle 2": "10.1%",
        "Muscle 3": "31.7%",
        "Robber": "12.7%",      // no number
        "Thief 1": "2.9%",
        "Thief 2": "29.1%"
      },
      "Clinical Precision": {
        "Cat Burglar": "18.9%", // no number
        "Assassin": "16.1%",
        "Cleaner": "21.7%",
        "Imitator": "43.3%"
      }
      ...
      let crimeName = _crimeArray[i].name;
      let roleName = _crimeArray[i].slots[j].position;
      ...
      if (numberedRoles[crimeName]?.includes(roleName)) {
          roleName += " " + _crimeArray[i].slots[j].position_number;
      }

      // Lookup soft CPR value
      let softCPR = softCPRRequirements[crimeName]?.[roleName] || "";

      // ===== CPR validation logic =====
      let cprStatus = "";
      if (_crimeSlotMemberStatus && _crimeSlotMemberStatus.trim() !== "") {
          let actualCPR = _crimeArray[i].slots[j].checkpoint_pass_rate || 0;

          // Special case: Picklock 2 in Blast from the Past
          if (crimeName === "Blast from the Past" &&
              _crimeArray[i].slots[j].position === "Picklock" &&
              _crimeArray[i].slots[j].position_number === 2) {

              if (actualCPR < 69) {
                  cprStatus = `<span style="color:green">Good</span>`;
              } else {
                  cprStatus = `<span style="color:orange">High CPR for Role</span>`;
              }

          } else if (softCPR) { // normal logic for other roles
              if (actualCPR >= softCPR) {
                  cprStatus = `<span style="color:green">Good</span>`;
              } else {
                  cprStatus = `<span style="color:red">CPR too low</span>`;
              }
          }
      }
            ...
            // ===== CPR delta logic =====
      let cprDelta = "";
      if (_crimeSlotMemberStatus && _crimeSlotMemberStatus.trim() !== "" && softCPR) {
          let actualCPR = _crimeArray[i].slots[j].checkpoint_pass_rate || 0;
          let diff = actualCPR - softCPR;

          // Clamp values between -15 and +15 for coloring scale
          let clamped = Math.max(-15, Math.min(15, diff));

          // Choose color based on diff
          let color = "#999"; // neutral grey
          if (clamped > 0) {
              // Green shades
              let greenIntensity = 100 + (clamped * 10); // goes from 110 → 250
              color = `rgb(0, ${greenIntensity}, 0)`;
          } else if (clamped < 0) {
              // Red shades
              let redIntensity = 100 + (Math.abs(clamped) * 10); // goes from 110 → 250
              color = `rgb(${redIntensity}, 0, 0)`;
          }

          cprDelta = `<span style="color:${color}">${diff > 0 ? "+" : ""}${diff}</span>`;
      }


      // ===== Mapped percent logic =====
      let mappedPercent = mappedPercentValues[crimeName]?.[roleName] || "";
      let mappedPercentDisplay = mappedPercent; // always show, even if slot empty
      ...
      _memberOutputHTML += (`<li class="table-cell OC2-crimeMemberLi OC2-crimeID_${_crimeArray[i].id} ${_userIndicatorClass}">
        ...
        <div class="OC2-tableCell OC2-tableCrimeMemberSoftCPR">${softCPR}</div>
        <div class="OC2-tableCell OC2-tableCrimeDelta">${cprDelta}</div>
        <div class="OC2-tableCell OC2-tableCrimeCPRStatus">${cprStatus}</div>
        <div class="OC2-tableCell OC2-tableCrimeMappedPercent">${mappedPercentDisplay}</div>
      </li>`);
 ...
 */
  ...
  $(".OC2-memberTable div.OC2-tableCrimeMemberSoftCPR").css({
    "width": "60px",        // adjust as needed
    "text-align": "center",
  })

  $(".OC2-memberTable div.OC2-tableCrimeDelta").css({
    "width": "70px",
    "text-align": "center",
    "font-weight": "bold"
  });
  ...
  $(".OC2-memberTable div.OC2-tableCrimeCPRStatus").css({
      "width": "80px",        // between 60-90px for balance
      "text-align": "center",
      "font-weight": "bold",
      "white-space": "nowrap",
      "font-family": "Arial, sans-serif",
  })
  $(".OC2-memberTable div.OC2-tableCrimeMappedPercent").css({
    "width": "80px",          // similar to CPR column for balance
    "text-align": "center",
    "font-weight": "bold",
    "white-space": "nowrap",
    "font-family": "Arial, sans-serif",

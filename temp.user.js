// ==UserScript==
// @name         Live Faction Attack Feed (Custom)
// @namespace    http://tampermonkey.net/
// @version      8.3
// @description  A highly customizable dashboard designed to provide real-time updates on your faction's attacks, making it an essential tool for monitoring Ranked Wars and Chain status.  
// @author       ANITABURN
...
(function() {
    ...
    const COLLECTED_RETALS_KEY = 'torn_faction_feed_collected_retals';
    ...
    const defaultSettings = {
        ...
        chainTimerSound: 'alarm',
        chainTimerThreshold: 15
    };
    ...
    if (settings.chainTimerEnabled === undefined) settings.chainTimerEnabled = defaultSettings.chainTimerEnabled;
    if (settings.chainTimerSound === undefined) settings.chainTimerSound = defaultSettings.chainTimerSound;
    if (settings.chainTimerThreshold === undefined) settings.chainTimerThreshold = defaultSettings.chainTimerThreshold;

    let apiKey = GM_getValue(API_KEY_KEY) || '';
    let myId = GM_getValue(MY_ID_KEY);
    let myFactionId = GM_getValue(MY_FAC_KEY);
    let collectedRetals = JSON.parse(GM_getValue(COLLECTED_RETALS_KEY, '{}'));
    ...
    function isPeoplePage() {
        return window.location.href.includes('page=people');
    }

    let chainTimerAlerted = false;
    let lastChainSeconds = Infinity;
    ...
    const ICONS = {
        ...
        chevronUp: `<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M7.41 15.41L12 10.83l4.59 4.58L18 14l-6-6-6 6z"/></svg>`,
        ...
        check: `<svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>`,
        chain: `<svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/></svg>`
    };
    ...
    function saveCollectedRetals() {
        const now = Date.now() / 1000;
        const cleaned = {};
        for (const [key, timestamp] of Object.entries(collectedRetals)) {
            if (now - timestamp < 600) {
                cleaned[key] = timestamp;
            }
        }
        collectedRetals = cleaned;
        GM_setValue(COLLECTED_RETALS_KEY, JSON.stringify(collectedRetals));
    }
    ...
    function getChainTimerSeconds() {
        const selectors = [
            '[class*="bar-timeleft"]',
            '[class*="timeleft"]',
            '.chain-bar .bar-timeleft',
            '#barChain [class*="timeleft"]'
        ];
        ...
        for (const sel of selectors) {
            ...
            if (el) {
                ...
                const seconds = parseTimerText(text);
    ...
    function parseTimerText(text) {
        if (!text || text === '' || text === '0:00') return null;
    ...
    function startChainTimerMonitor() {
        stopChainTimerMonitor();
        chainTimerAlerted = false;
        lastChainSeconds = Infinity;

        chainTimerInterval = setInterval(() => {
            const seconds = getChainTimerSeconds();

            updateChainTimerDisplay(seconds);
            updateMiniChainDisplay(seconds);

            if (!settings.chainTimerEnabled || settings.chainTimerSound === 'none') return;

            if (seconds === null) {
                if (chainTimerAlerted) chainTimerAlerted = false;
                lastChainSeconds = Infinity;
                return;
            }

            const threshold = settings.chainTimerThreshold;

            if (seconds <= threshold && lastChainSeconds > threshold && !chainTimerAlerted) {
                chainTimerAlerted = true;
                playSound(settings.chainTimerSound);
            }
            ...
            if (seconds <= Math.min(threshold, 10) && seconds > 0) {
                if (seconds % 5 <= 1) {
                    playSound(settings.chainTimerSound);
                }
            }

            lastChainSeconds = seconds;
        }, 1000);
    }

    function stopChainTimerMonitor() {
        if (chainTimerInterval) clearInterval(chainTimerInterval);
        chainTimerInterval = null;
    }

    function updateChainTimerDisplay(seconds) {
        const display = document.getElementById('chain-timer-display');
        if (!display) return;

        if (!settings.chainTimerEnabled) {
        ...
        const threshold = settings.chainTimerThreshold;
        let color = settings.winColor;
        let flash = false;

        if (seconds <= threshold && seconds > Math.floor(threshold / 2)) {
            color = settings.stalemateColor;
        } else if (seconds <= Math.floor(threshold / 2) && seconds > 0) {
            color = settings.lossColor;
            flash = true;
        }

        display.innerHTML = `${ICONS.chain} <span style="color:${color}; font-weight:bold; font-family:monospace;">${timeStr}</span>`;
        display.style.color = color;

        if (flash) {
            display.classList.add('chain-flash');
        } else {
            display.classList.remove('chain-flash');
        }
    }

    function updateMiniChainDisplay(seconds) {
        if (isMobile) return;
        const el = document.querySelector('.mini-chain');
        if (!el) return;
        if (seconds === null || !settings.chainTimerEnabled) {
        ...
        const s = seconds % 60;
        const color = seconds <= settings.chainTimerThreshold ? settings.lossColor : (seconds <= settings.chainTimerThreshold * 2 ? settings.stalemateColor : settings.winColor);
    ...
    function createStyles() {
        ...
        const cursor = settings.floatMode && !isMobile ? 'move' : 'default';
        const mobileFontSize = 12;
        const mobilePadding = '8px 12px';

        const lossRgb = hexToRgb(settings.lossColor);
        const winRgb = hexToRgb(settings.winColor);
        ...
        const css = `
            ...
            #chain-timer-display.chain-flash {
                animation: chainFlash 0.5s ease-in-out infinite alternate;
            }
            @keyframes chainFlash {
                0% { background: rgba(${lossRgb}, 0.1); }
                100% { background: rgba(${lossRgb}, 0.35); }
            }
            #retal-btn {
                background: #333;
                color: #aaa;
            ...
            #retal-btn.active {
                background: ${settings.lossColor};
                color: #fff;
                border-color: ${settings.lossColor};
                animation: pulse 2s infinite;
            }
            @keyframes pulse {
                0% { box-shadow: 0 0 0 0 rgba(${lossRgb}, 0.4); }
                70% { box-shadow: 0 0 0 6px rgba(${lossRgb}, 0); }
                100% { box-shadow: 0 0 0 0 rgba(${lossRgb}, 0); }
            }
            .header-icon-btn {
                cursor: pointer;
                font-size: 14px;
            ...
            #attack-feed-scroll::-webkit-scrollbar {
                width: 6px;
            }
            #attack-feed-scroll::-webkit-scrollbar-thumb {
                background: #555;
                border-radius: 3px;
            }
            #attack-feed-retals {
                display: none;
                border-bottom: 2px solid ${settings.lossColor};
                background: ${settings.bgColor};
                max-height: 150px;
                overflow-y: auto;
                flex-shrink: 0;
            }
            #attack-feed-retals::-webkit-scrollbar {
                width: 4px;
            }
            #attack-feed-retals::-webkit-scrollbar-thumb {
                background: ${settings.lossColor};
                border-radius: 2px;
            }
            #attack-feed-content {
            ...
            .retal-item {
                ...
                justify-content: space-between;
                align-items: center;
                border-left: 3px solid ${settings.lossColor};
                background: rgba(30,30,30,0.5);
            }
            .retal-item.collected {
                opacity: 0.5;
                border-left-color: ${settings.winColor};
            ...
            .retal-item.collected .retal-target {
                color: ${settings.winColor};
            }
            .retal-victim {
                font-size: 0.85em;
            ...
            .retal-collected-badge {
                background: ${settings.winColor};
                color: #000;
                padding: 2px 6px;
                border-radius: 3px;
                font-size: 10px;
            ...
            .settings-group input:not([type="checkbox"]):not([type="color"]),
            ...
            .settings-group.block label {
                display: block;
                margin-bottom: 4px;
            }
            .settings-group.block input {
                width: 100%;
            }
            .settings-buttons {
                display: flex;
            ...
            .btn-save { background: ${settings.winColor}; color: #000; border: none; }
            .btn-save:hover { opacity: 0.85; }
            .btn-reset { background: ${settings.lossColor}; color: white; border: none; }
            ...
            @media screen and (max-width: 768px) {
                ...
                #attack-feed-box.collapsed #attack-feed-scroll,
                #attack-feed-box.collapsed #attack-feed-resize-handle,
                #attack-feed-box.collapsed #feed-filter-menu {
                ...
                #attack-feed-box.collapsed #attack-feed-header {
                    border-radius: 5px;
                    border-bottom: none;
                }
                .header-left {
            ...
            .header-icon-btn, #retal-btn, #collapse-btn, .btn-alert {
                -webkit-tap-highlight-color: transparent;
    ...
    function createUI() {
        ...
        box.innerHTML = `
            <div id="attack-feed-header">
                <div class="header-left">
                    ...
                    <div id="chain-timer-display"></div>
                </div>
                <div class="header-controls">
                    ...
                    <button id="collapse-btn" title="Collapse">${ICONS.chevronDown}</button>
                </div>
            </div>
        ...
        document.getElementById('attack-feed-status').addEventListener('click', (e) => {
            e.stopPropagation();
            fetchAttacks();
        });

        document.getElementById('attack-feed-filter-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            toggleFilterMenu();
        });

        document.getElementById('retal-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            toggleRetalView();
        });
        ...
        if (isMobile) {
            document.getElementById('attack-feed-header').addEventListener('click', (e) => {
                if (e.target.tagName === 'BUTTON' || e.target.classList.contains('header-icon-btn') || e.target.closest('.header-icon-btn') || e.target.closest('button')) return;
                toggleMobileCollapse();
            });
        }
        ...
        document.addEventListener('click', (e) => {
            if (isFilterOpen) {
                ...
                const filterBtn = document.getElementById('attack-feed-filter-btn');
    ...
    function toggleMobileCollapse() {
        const box = document.getElementById('attack-feed-box');
        const collapseBtn = document.getElementById('collapse-btn');
        if (!box || !collapseBtn) return;

        isPanelCollapsed = !isPanelCollapsed;
        box.classList.toggle('collapsed', isPanelCollapsed);
        collapseBtn.innerHTML = isPanelCollapsed ? ICONS.chevronUp : ICONS.chevronDown;

        if (!isPanelCollapsed) {
            box.dataset.userExpanded = 'true';
        } else {
            delete box.dataset.userExpanded;
        }
    }
    ...
    function setupDrag(box) {
        ...
        const startDrag = (clientX, clientY) => {
            if (!settings.floatMode || isMobile) return false;
            isDragging = true;
            dragOffset.x = clientX - box.offsetLeft;
            dragOffset.y = clientY - box.offsetTop;
            return true;
        };

        const doDrag = (clientX, clientY) => {
            if (isDragging && settings.floatMode && !isMobile) {
        ...
        const endDrag = () => {
            if (isDragging && settings.floatMode && !isMobile) {
                isDragging = false;
                const posData = { top: box.style.top, left: box.style.left, right: 'auto' };
                GM_setValue(BOX_POS_KEY, JSON.stringify(posData));
            }
        };

        // Mouse drag
        header.addEventListener('mousedown', (e) => {
            if (e.target.tagName === 'BUTTON' || e.target.classList.contains('header-icon-btn') || e.target.closest('.header-icon-btn') || e.target.closest('button') || e.target.id === 'attack-feed-status') return;
        ...
        // Touch drag (for floatMode on tablet / touch devices)
        header.addEventListener('touchstart', (e) => {
            if (!settings.floatMode || isMobile) return;
            if (e.target.tagName === 'BUTTON' || e.target.classList.contains('header-icon-btn') || e.target.closest('.header-icon-btn') || e.target.closest('button') || e.target.id === 'attack-feed-status') return;
            const t = e.touches[0];
    ...
    function initTrigger() {
        ...
        // On the people page (no war list), show a fixed floating toggle button
        if (isPeoplePage()) {
            initPeoplePageTrigger();
            return;
        }
        ...
        const attach = () => {
            ...
            if (!container) {
                // Fallback: if no war list found on any page, use floating toggle
                if (!document.getElementById('feed-float-toggle')) {
                    initFloatingToggle();
                }
                return;
            }
            ...
            if (!trigger.dataset.feedAttached || trigger.innerHTML.trim() === "") {
                ...
                trigger.onclick = (e) => {
                    if (e.target.closest('.mini-settings-btn')) return;
                    e.preventDefault();
                    e.stopPropagation();
    ...
    function initPeoplePageTrigger() {
        // On the people page there's no war list widget, so we create a
        // fixed floating toggle button in the top-right corner instead.
        initFloatingToggle();

        // Auto-show the feed panel on the people page (floatMode)
        const box = document.getElementById('attack-feed-box');
        if (box && box.style.display !== 'flex') {
            box.style.display = 'flex';
            box.style.position = 'fixed';
            const saved = JSON.parse(GM_getValue(BOX_POS_KEY, '{"top":"60px","right":"10px","left":"auto"}'));
            box.style.top = saved.top || '60px';
            // Prefer right-anchored position on people page
            if (saved.right && saved.right !== 'auto') {
                box.style.right = saved.right;
                box.style.left = 'auto';
            } else {
                box.style.left = saved.left || 'auto';
            }
        }
    }

    function initFloatingToggle() {
        if (document.getElementById('feed-float-toggle')) return;

        const btn = document.createElement('div');
        btn.id = 'feed-float-toggle';
        btn.title = 'Toggle Faction Feed';
        btn.innerHTML = `${ICONS.sword} <span style="font-size:11px; font-weight:700; letter-spacing:0.5px;">FEED</span>`;
        btn.style.cssText = `
            position: fixed;
            top: 10px;
            right: 10px;
        ...
        btn.addEventListener('click', (e) => {
            ...
            if (box.style.display === 'flex') {
                ...
                box.style.display = 'flex';
                box.style.position = 'fixed';
                const saved = JSON.parse(GM_getValue(BOX_POS_KEY, '{"top":"55px","left":"auto"}'));
                box.style.top = saved.top || '55px';
                if (saved.right && saved.right !== 'auto') {
                    box.style.right = saved.right;
                    box.style.left = 'auto';
                } else if (saved.left && saved.left !== 'auto') {
    ...
    function initMobilePanel() {
        ...
        let mobileInsertPending = false;

        const insertPanel = () => {
            if (mobileInsertPending) return;
            if (box.dataset.mobileInserted === 'true' && document.contains(box) && box.parentElement !== document.body) {
                return;
            }

            mobileInsertPending = true;

            const fWarList = document.querySelector('[class*="faction_war_list"]') || document.querySelector('.f-war-list');
            const redMsg = document.querySelector('.f-msg.m-top10.red') || document.querySelector('.f-msg.red');
            const chainBoxTitle = document.querySelector('.chain-box-title-block');

            let insertTarget = null;
            let insertMode = 'before'; 

            if (fWarList) {
                insertTarget = fWarList;
                insertMode = 'before';
            } else if (chainBoxTitle) {
                insertTarget = chainBoxTitle;
                insertMode = 'before';
            }

            if (redMsg && insertTarget) {
                const redRect = redMsg.compareDocumentPosition(insertTarget);
                if (redRect & Node.DOCUMENT_POSITION_FOLLOWING) {
                    insertTarget = redMsg;
                    insertMode = 'after';
                }
            } else if (redMsg && !insertTarget) {
                insertTarget = redMsg;
                insertMode = 'after';
            }
            ...
            if (insertTarget) {
                box.style.display = 'flex';
                if (!box.dataset.userExpanded) {
                    box.classList.add('collapsed');
                    isPanelCollapsed = true;
                }
                if (insertMode === 'after') {
                    insertTarget.parentNode.insertBefore(box, insertTarget.nextSibling);
                } else {
                    insertTarget.parentNode.insertBefore(box, insertTarget);
                }
                box.dataset.mobileInserted = 'true';
            } else {
                // No insertion target found — fall back to fixed top bar (common on people page / mobile)
                box.style.display = 'flex';
                box.style.position = 'fixed';
        ...
        const observer = new MutationObserver(() => {
            ...
            if (box.parentElement === document.body || !document.contains(box)) {
                box.dataset.mobileInserted = 'false';
                insertPanel();
            }
        });
        ...
        setInterval(() => {
            const box = document.getElementById('attack-feed-box');
            if (box && (box.parentElement === document.body || !document.contains(box))) {
                box.dataset.mobileInserted = 'false';
                insertPanel();
            }
        }, 3000);
    ...
    function updateTriggerContent() {
        ...
        const retalCount = activeRetals.filter(r => !isRetalCollected(r)).length;

        const chainSec = getChainTimerSeconds();
        let chainStr = '';
        if (chainSec !== null && settings.chainTimerEnabled) {
            const m = Math.floor(chainSec / 60);
            const s = chainSec % 60;
            const color = chainSec <= settings.chainTimerThreshold ? settings.lossColor : (chainSec <= settings.chainTimerThreshold * 2 ? settings.stalemateColor : settings.winColor);
            chainStr = `<span class="mini-chain" style="color:${color}">⛓ ${m}:${s.toString().padStart(2, '0')}</span>`;
        }

        const gearBtn = `<span class="mini-settings-btn" title="Settings">${ICONS.gear}</span>`;

        let html = '';

        if (!last) {
            html = `<div class="mini-feed-container" style="justify-content:center; align-items:center; color:#888;">${gearBtn}Waiting for data...</div>`;
        } else {
            ...
            html = `
                <div class="mini-feed-container">
                    ...
                    <div class="mini-row-top">
                        <span class="mini-attacker" style="color:${settings.assistColor}">${attName}</span>
        ...
        if (lastTriggerHTML !== html || trigger.innerHTML.trim() === "") {
            ...
            const miniGear = trigger.querySelector('.mini-settings-btn');
            if (miniGear) {
                miniGear.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    showSettingsModal();
                });
    ...
    function renderFilterMenu() {
        ...
        const memberOptions = Object.entries(factionMembers)
            .sort((a, b) => a[1].localeCompare(b[1]))
            .map(([id, name]) => `<option value="${id}" ${settings.filterAttacker == id ? 'selected' : ''}>${name}</option>`)
            .join('');
        ...
        document.getElementById('filter-result').onchange = (e) => {
            settings.filterResults = e.target.value;
            saveSettings();
            renderFeed(lastAttacks, false);
        };
        document.getElementById('filter-attacker').onchange = (e) => {
            settings.filterAttacker = e.target.value;
            saveSettings();
            renderFeed(lastAttacks, false);
        };
        document.getElementById('filter-type').onchange = (e) => {
            settings.filterAttackType = e.target.value;
            saveSettings();
            renderFeed(lastAttacks, false);
        };
    }
    ...
    function setupResize(box) {
        ...
        document.addEventListener('mousemove', (e) => {
            if (isResizing) {
                const nw = resizeStart.w + (e.clientX - resizeStart.x);
                const nh = resizeStart.h + (e.clientY - resizeStart.y);
                if (nw > 200) {
                    settings.boxWidth = nw;
                    box.style.width = nw + 'px';
                }
                if (nh > 150) {
                    settings.boxHeight = nh;
    ...
    function identifySelf(key) {
        ...
        GM_xmlhttpRequest({
            method: "GET",
            url: `https://api.torn.com/user/?selections=profile&key=${key}&_=${Date.now()}`,
            onload: function(res) {
                try {
                    const d = JSON.parse(res.responseText);
                    if (d.player_id) {
                        myId = d.player_id;
                        myFactionId = d.faction ? d.faction.faction_id : 0;
                        GM_setValue(MY_ID_KEY, myId);
                        GM_setValue(MY_FAC_KEY, myFactionId);
                        fetchFactionMembers(key);
                        fetchAttacks();
                    }
                } catch (e) {}
    ...
    function fetchFactionMembers(key) {
        GM_xmlhttpRequest({
            method: "GET",
            url: `https://api.torn.com/faction/?selections=basic&key=${key}&_=${Date.now()}`,
            onload: function(res) {
                try {
                    const d = JSON.parse(res.responseText);
                    if (d.members) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                try {
                    ...
                    checkCollectedRetals(data.attacks);
    ...
    function checkForNewRetals(current, old) {
        ...
        for (const id of newIds) {
            ...
            if (att.attacker_faction != myFactionId && ['Hospitalized', 'Mugged', 'Attacked', 'Arrested'].includes(att.result)) {
                return true;
            }
        }
        return false;
    ...
    function isRetalCollected(att) {
        const retalKey = `${att.attacker_id}_${att.timestamp_ended}`;
        return collectedRetals.hasOwnProperty(retalKey);
    }

    function markRetalCollected(att) {
        const retalKey = `${att.attacker_id}_${att.timestamp_ended}`;
        collectedRetals[retalKey] = Date.now() / 1000;
        saveCollectedRetals();
    }

    function checkCollectedRetals(attacks) {
        if (!attacks) return;

        const now = Date.now() / 1000;

        for (const att of Object.values(attacks)) {
            if (now - att.timestamp_ended > 600) continue;
            if (att.attacker_faction != myFactionId) continue;
            if (!['Hospitalized'].includes(att.result)) continue;

            for (const retal of activeRetals) {
                if (att.defender_id == retal.attacker_id) {
                    if (att.timestamp_ended > retal.timestamp_ended) {
                        markRetalCollected(retal);
                    }
                }
            }
        }
    ...
    function updateRetalButton() {
        ...
        const uncollectedCount = activeRetals.filter(r => !isRetalCollected(r)).length;

        btn.innerText = `Retal: ${uncollectedCount}`;
    ...
    function renderRetalList() {
        ...
        const now = Date.now() / 1000;
        const validRetals = activeRetals.filter(att => (now - att.timestamp_ended) <= 300);

        if (validRetals.length === 0) {
            content.innerHTML = '<div style="padding:10px;text-align:center;color:#888;font-size:11px;">No active retaliations.<br>(Incoming losses < 5m ago)</div>';
            return;
        }

        validRetals.forEach(att => {
            ...
            const isCollected = isRetalCollected(att);

            const div = document.createElement('div');
            div.className = 'retal-item' + (isCollected ? ' collected' : '');

            if (isCollected) {
                div.innerHTML = `
                    <div class="retal-info">
                        <a href="https://www.torn.com/profiles.php?XID=${att.attacker_id}" target="_blank" class="retal-target">${ICONS.check} ${att.attacker_name} [${att.attacker_id}]</a>
                        <div class="retal-victim">Beat: ${att.defender_name}</div>
                    </div>
                    <div class="retal-actions">
                        <span class="retal-collected-badge">${ICONS.check} COLLECTED</span>
                    </div>
                `;
            } else {
                div.innerHTML = `
                ...
                const alertBtn = div.querySelector('.btn-alert');
                if (alertBtn) {
                    alertBtn.onclick = function() {
                        sendAlert(att, timeStr);
                        this.style.color = settings.winColor;
                    };
                }
            }
    ...
    function startRetalTimer() {
        ...
        retalTimerInterval = setInterval(() => {
            ...
            const now = Date.now() / 1000;
            timers.forEach(el => {
                const endTime = parseFloat(el.getAttribute('data-end'));
                const left = endTime - now;
                if (left <= 0) {
                    const item = el.closest('.retal-item');
                    if (item) item.remove();
                } else {
                    el.innerText = `${Math.floor(left / 60).toString().padStart(2, '0')}:${Math.floor(left % 60).toString().padStart(2, '0')}`;
                }
            });
    ...
    function stopRetalTimer() {
        if (retalTimerInterval) clearInterval(retalTimerInterval);
    }
    ...
    function renderFeed(attacksData, hasNewAttacks) {
        ...
        if (!attacksData || Object.keys(attacksData).length === 0) {
            content.innerHTML = '<div style="padding:20px;text-align:center;">No data</div>';
            return;
        }
        ...
        attacks.forEach((att, index) => {
            ...
            if (att.result === 'Assist') {
                type = 'assist';
            } else if (isMyFac) {
                type = success ? 'win' : (att.result !== 'Stalemate' ? 'loss' : 'stalemate');
            } else {
                type = success ? 'loss' : (att.result !== 'Stalemate' ? 'win' : 'stalemate');
            }
            ...
            let resHtml = '';
            if (settings.showRespect && att.respect) {
                resHtml = `<span style="color:${isMyFac ? settings.winColor : settings.lossColor}; font-weight:bold;">${isMyFac ? '+' : '-'}${att.respect.toFixed(2)} R</span>`;
            }
            ...
            const attackerHtml = settings.showAttacker ? `<div style="font-size:11px; color:${settings.assistColor}; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${attName}${attIdStr}</div>` : '';

            html += `
                <div class="feed-item ${cssClass} ${highlight}">
                    <div class="feed-info">
                        ${attackerHtml}
                        <div style="font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${att.defender_name} [${att.defender_id}]</div>
    ...
    function showSettingsModal() {
        ...
        const chainSoundOpts = `
            ...
            <option value="chime">Chime</option>
            <option value="pulse">Pulse</option>
            <option value="urgentAlarm">Urgent Alarm</option>
        `;
        ...
        modal.innerHTML = `
            ...
            <div class="settings-section">
                <h4>Display</h4>
                ${!isMobile ? '<div class="settings-group"><label>Float Mode</label><input type="checkbox" id="setting-float-mode" ' + (settings.floatMode ? 'checked' : '') + '></div>' : ''}
                ...
                <div class="settings-group"><label>Highlight New</label><input type="checkbox" id="setting-highlight" ${settings.highlightNewAttacks ? 'checked' : ''}></div>
            </div>
            <div class="settings-section">
                <h4>Notifications</h4>
                ...
                <button class="btn-test" id="btn-test-sound">Test Attack / Retal Sound</button>
            </div>
            <div class="settings-section">
                <h4>⛓ Chain Timer</h4>
                <div class="settings-group"><label>Chain Timer Enabled</label><input type="checkbox" id="setting-chain-enabled" ${settings.chainTimerEnabled ? 'checked' : ''}></div>
                <div class="settings-group"><label>Alert Sound</label>
                    <select id="setting-chain-sound">${chainSoundOpts}</select>
                </div>
                <div class="settings-group"><label>Alert When ≤ (sec)</label><input type="number" id="setting-chain-threshold" value="${settings.chainTimerThreshold}" min="5" max="120"></div>
                <div style="font-size:11px; color:#888; margin-top:4px; padding:0 2px;">
                    Plays alert sound when chain timer drops below the threshold. Syncs with Torn's chain bar timer. Repeats every 5s if critically low (≤10s).
                </div>
                <button class="btn-test" id="btn-test-chain-sound">Test Chain Sound</button>
            </div>
            <div class="settings-section">
                <h4>Colors</h4>
        ...
        document.getElementById('setting-chain-sound').value = settings.chainTimerSound;
        ...
        document.getElementById('setting-chain-sound').onchange = (e) => playSound(e.target.value);

        document.getElementById('btn-test-sound').onclick = (e) => {
            e.stopPropagation();
            playSound(document.getElementById('setting-sound-retal').value);
        };
        document.getElementById('btn-test-chain-sound').onclick = (e) => {
            e.stopPropagation();
            playSound(document.getElementById('setting-chain-sound').value);
        };
        ...
        document.getElementById('btn-save').onclick = () => {
            try {
                ...
                if (!isMobile) {
                    const floatModeEl = document.getElementById('setting-float-mode');
                    if (floatModeEl) settings.floatMode = floatModeEl.checked;
                }
                ...
                settings.chainTimerEnabled = document.getElementById('setting-chain-enabled').checked;
                settings.chainTimerSound = document.getElementById('setting-chain-sound').value;
                settings.chainTimerThreshold = Math.max(5, Math.min(120, parseInt(document.getElementById('setting-chain-threshold').value) || 15));
